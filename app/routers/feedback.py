...
def create_feedback_route(feedback: FeedbackCreate, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
...
def update_feedback_route(feedback_id: int, feedback: FeedbackUpdate, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    db_feedback = update_feedback(db, feedback_id, feedback, current_user.id)
    if not db_feedback:
        raise HTTPException(status_code=404, detail="Feedback not found")
    return db_feedback

@router.delete("/feedback/{feedback_id}", response_model=FeedbackResponse)
def delete_feedback_route(feedback_id: int, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_user)):
    db_feedback = delete_feedback(db, feedback_id, current_user.id)
    if not db_feedback:
        raise HTTPException(status_code=404, detail="Feedback not found")
    return db_feedback
